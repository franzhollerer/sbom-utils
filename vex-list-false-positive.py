...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description='List false positive.'
        )
...
def list_false_positive(vex):
    for vul in vex['vulnerabilities']:
        if vul['analysis']['state'] == 'false_positive':
            print(f"{vul['properties'][0]['value']}:")
            print(f"    Id: {vul['id']}")
            print()
            s = wrap_and_indent('Description: ' + vul['description'])
            print(s)
            if 'detail' in vul['analysis']:
                s = wrap_and_indent('Detail: ' + vul['analysis']['detail'])
                print()
                print(s)
            print()
